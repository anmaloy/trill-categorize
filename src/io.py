...
import numpy as np
import src.readSGLX as readSGLX
...
def load_phy(ks_dir, use_label='intersect'):
    ...
    "cell_id" is a remapped identifier for the unit a spike came from. cell_id maps one to one onto cluster_id,
    but takes values 0 to N, where N is the number of unique units

    "cell_id" is used after filtering out bad units, and resorting by depth

    The metrics data frame gives cluster level information (such as probe location, spike amplitude, firing rate,
    QC metrics etc...)
    """
